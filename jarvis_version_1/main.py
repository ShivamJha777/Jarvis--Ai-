...
def Search_Google(query):
    ...
    from Jarvis_version_2.speechoffline import Speak
...
def Search_Youtube(query):
    ...
    try:
        ...
        webbrowser.open(web)
        speak('Boss this is what I found on youtube want me to open the video which I think is the most relative to the topic?')
        choice = Listen().lower()
        if 'yes' in choice or 'sure' in choice or 'why not' in choice or 'would love to' in choice:
...
lives = 5
for i in range(5):
    say('To Start type the Password')
    ...
    if 'jarvis' in query:
        if 'the time'.lower() in query:
            ...
            try:
                ...
                from features.Fun_fact import get_fun_fact
            ...
            say(f'The download speed is {download} megabytes per second')
            ...
            from features.jokes import present_joke
            if 'one' in query:
                present_joke('one')
            else:
            ...
            from features.NewsRead import latestenews
            ...
            if 'in' in query:
                ...
                query = query.replace('jarvis', '')
                search = query
            from features.Weather_and_Temperature import Temperature_finder

            Temperature_finder(search)
        elif 'weather' in query:
            from features.Weather_and_Temperature import find_weather
